...
def _clean(value: Any) -> Any:
    if isinstance(value, dict):
        return {
            ...
            if v is not None and _clean(v) not in ({}, [])
        }
...
def _timing(
    use: str | None, time_min: float | None, temperature_c: float | None = None
) -> dict[str, Any]:
    return _clean(
        {
            "use": use,
            "time": _measure(time_min, "min") if time_min is not None else None,
            "temperature": _measure(temperature_c, "C")
            ...
            _clean(
                {
                    ...
                    "timing": _timing(f.use, None),
                    "amount": f.quantity.measure() if f.quantity else None,
                    "yield": _measure(f.yield_pct, "%")
                    if f.yield_pct is not None
                    else None,
                    "color": _measure(f.color_srm, "SRM")
                    if f.color_srm is not None
                    else None,
                }
            )
            ...
            _clean(
                {
                    ...
                    "alpha_acid": _measure(h.alpha, "%")
                    if h.alpha is not None
                    else None,
                    "amount": h.quantity.measure() if h.quantity else None,
            ...
            _clean(
                {
                    ...
                    "amount": m.quantity.measure() if m.quantity else None,
            ...
            _clean(
                {
                    ...
                    "timing": _timing(y.use, None),
                    "product_id": y.product_id,
                    "type": y.type or "Ale",
                    "form": y.form,
                    "attenuation": _measure(y.attenuation, "%")
                    if y.attenuation is not None
                    else None,
                    "amount": y.quantity.measure() if y.quantity else None,
                }
            )
            for y in r.yeasts
        ],
                ...
                _clean(
                    {
                        ...
                        "step_time": _measure(s.step_time_min, "min")
                        if s.step_time_min is not None
                        else None,
                        "step_temperature": _measure(s.step_temp_c, "C")
                        if s.step_temp_c is not None
                        else None,
                        "ramp_time": _measure(s.ramp_time_min, "min")
                        if s.ramp_time_min is not None
                        else None,
                        "end_temperature": _measure(s.end_temp_c, "C")
                        if s.end_temp_c is not None
                        else None,
                    }
                )
    ...
    return _clean(
        {
            ...
            "boil_size": _measure(r.boil_size_l, "l"),
            "efficiency": {"brewhouse": _measure(r.efficiency_pct, "%")},
            "style": style,
            ...
            "alcohol_by_volume": _measure(r.est_abv, "%")
            if r.est_abv is not None
            else None,
            "ibu_estimate": _measure(r.ibu, "IBUs") if r.ibu is not None else None,
            "color_estimate": _measure(r.est_color_srm, "SRM")
            if r.est_color_srm is not None
            else None,
            "boil": {"name": "Boil", "duration": _measure(r.boil_time_min, "min")}
            if r.boil_time_min is not None
            else None,
        }
    )
